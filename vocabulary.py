...
class BERTVocabulary(object):
    ...
    def build_vocab(self):
        token = []
        with open("./bert_model/vocab.txt", "r", encoding="utf-8") as file:
            for line in file:
                word = line.strip()
                token.append(word)
        self.word2idx = {word: index for index, word in enumerate(token)}
        self.idx2word = {index: word for index, word in enumerate(token)}
